...
export default function MovementLibrary({
  ...
  const displayedMovements = hasSearch ? searchResults : initialMovements;

  const displayedIsSearching = hasSearch && isSearching;

  const displayedError = hasSearch ? error : null;
  ...
  return (
    <>
      ...
      {displayedMovements.length === 0 && !displayedIsSearching ? (
