...
type ProgressResult = TrendResult;
...
type WorkoutProgressTrack = {
  ...
  personalBest: ProgressResult | null;

  latestResult: ProgressResult;

  firstResult: ProgressResult;
...
type WorkoutProgressResponse = {
  ...
  tracks: WorkoutProgressTrack[];
};

type MovementProgressResult = MovementTrendResult & {
  measurementType: {
    key: string;
    name: string;
  };
...
type MovementProgressTrack = {
  ...
  personalBest: MovementProgressResult | null;

  latestResult: MovementProgressResult;

  firstResult: MovementProgressResult;

  history: MovementProgressResult[];
};
...
type MovementProgressResponse = {
  ...
  tracks: MovementProgressTrack[];
};

const emptyWorkoutProgress: WorkoutProgressResponse = {
  summary: {
    totalResults: 0,
    uniqueWorkouts: 0,
    rxResults: 0,
...
const emptyMovementProgress: MovementProgressResponse = {
  summary: {
    totalResults: 0,
    uniqueMovements: 0,
    personalRecords: 0,
...
async function getWorkoutProgress(): Promise<WorkoutProgressResponse> {
  return authenticatedApiFetchJson<WorkoutProgressResponse>(
    "/workouts/results/progress",
  );
}

async function getMovementProgress(): Promise<MovementProgressResponse> {
  return authenticatedApiFetchJson<MovementProgressResponse>(
...
export default async function ProgressPage() {
  const [
    ...
    getLocale(),
    Promise.allSettled([getWorkoutProgress(), getMovementProgress()]),
  ]);

  const [workoutProgressResult, movementProgressResult] = progressResults;

  if (
    workoutProgressResult.status === "rejected" &&
    movementProgressResult.status === "rejected"
  ) {
    throw new Error("Unable to load progress data");
  }

  const workoutProgressFailed = workoutProgressResult.status === "rejected";
  const movementProgressFailed = movementProgressResult.status === "rejected";

  const workoutProgress = workoutProgressFailed
    ? emptyWorkoutProgress
    : workoutProgressResult.value;

  const movementProgress = movementProgressFailed
    ? emptyMovementProgress
    : movementProgressResult.value;

  const workoutSummary = workoutProgress.summary;

  const workoutTracks = workoutProgress.tracks;

  const movementSummary = movementProgress.summary;

  const movementTracks = movementProgress.tracks;

  const totalResults =
    workoutSummary.totalResults + movementSummary.totalResults;
  ...
  function getMeasurementName(key: string, fallback: string) {
    const translationKey = key.toLowerCase();

    return measurementT.has(translationKey)
      ? measurementT(translationKey)
      : fallback;
  }

  function getCategoryName(key: string, fallback: string) {
    const translationKey = key.toLowerCase();

    return categoryT.has(translationKey) ? categoryT(translationKey) : fallback;
  }

  function formatWorkoutResult(result: ProgressResult | null) {
    if (!result) {
      return "—";
    }
  ...
  function formatMovementResult(result: MovementProgressResult | null) {
    if (!result) {
      return "—";
    }
  ...
  function getWorkoutImprovement(workout: WorkoutProgressTrack) {
    if (workout.attemptCount < 2) {
      return null;
    }

    const first = workout.firstResult;

    const best = workout.personalBest;

    if (!best) {
      return null;
    }

    switch (workout.resultType.key) {
      case "TIME": {
        if (
          first.timeSeconds === null ||
        ...
        const difference = first.timeSeconds - best.timeSeconds;
        ...
        return {
          ...
          label:
            difference > 0
              ? t("improvement.faster", {
                  value: formatDuration(difference),
                })
              : difference === 0
                ? t("improvement.noChange")
                : t("improvement.slower", {
                    value: formatDuration(Math.abs(difference)),
                  }),

          percentage: Math.abs((difference / first.timeSeconds) * 100),
        };
      }

      case "REPS": {
        if (first.reps === null || best.reps === null) {
          return null;
        }

        const difference = best.reps - first.reps;
        ...
        return {
          ...
          label:
            difference > 0
              ? t("improvement.repsGain", {
                  count: difference,
                })
              : difference === 0
                ? t("improvement.noChange")
                : t("improvement.repsLoss", {
                    count: Math.abs(difference),
                  }),

          percentage:
            first.reps > 0 ? Math.abs((difference / first.reps) * 100) : null,
        };
      }
