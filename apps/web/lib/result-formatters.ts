...
export function formatWeight(
  ...
  return `${load}${weightUnit ? ` ${weightUnit}` : ""}`;
}
...
export function formatWorkoutResult(
  ...
  const { formatReps = (reps) => String(reps) } = options;
  ...
  switch (result.resultType.key) {
    ...
    case "LOAD":
      return result.load !== null
        ? formatWeight(result.load, result.weightUnit)
        : EMPTY_RESULT_VALUE;
...
export function formatMeasurementResult(
  ...
  switch (measurementTypeKey) {
    case "WEIGHT": {
      ...
      const load = formatWeight(result.load, result.weightUnit);

      return result.reps !== null ? `${result.reps} × ${load}` : load;
    }
...
export function formatPerformedMovement(
  movement: Pick<
    PerformedMovement,
    "reps" | "load" | "weightUnit" | "distance" | "durationSeconds" | "calories"
  >,
  options: FormatWorkoutResultOptions = {},
): string {
  const { formatReps = (reps) => String(reps) } = options;
  ...
  if (movement.reps !== null) {
    values.push(formatReps(movement.reps));
  }

  if (movement.load !== null) {
    values.push(formatWeight(movement.load, movement.weightUnit));
  }
