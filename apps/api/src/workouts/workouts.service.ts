...
export class WorkoutsService {
  ...
  async findResultProgress(userId: string) {
    ...
    const tracks = Array.from(groupedResults.values()).map((trackResults) => {
      ...
      const resultType =
        latestResult.workout.type.defaultResultType ??
    ...
    return {
      summary: {
        ...
        rxRate:
          totalResults > 0
            ? Math.round((rxResults / totalResults) * 100)
            : 0,
  ...
  async createResult(
    ...
    if (
      new Set(submittedMovementIds).size !==
    ...
    const workoutMovementMap = new Map(
      workoutVariant.sections.flatMap((section) =>
        section.movements.map(
          (movement) => [movement.id, movement] as const,
        ),
      ),
    );
  ...
  private async buildGeneratedMovementResults(
    ...
    const measurementTypeMap = new Map<string, string>(
      measurementTypes.map(
        (measurementType: { id: string; key: string }) => [
          measurementType.key,
          measurementType.id,
        ],
      ),
      ...
      variants: workout.variants.map((variant: any) => ({
        ...
        sections: variant.sections.map((section: any) => ({
          ...
          movements: section.movements.map((item: any) => ({
            ...
            prescriptions: item.prescriptions.map(
              (prescription: any) => ({
                id: prescription.id,
      ...
      case 'LOAD':
        return (
          [...validResults]
            ...
            .sort(
              (a, b) =>
                this.normalizeLoadToKg(b) -
                this.normalizeLoadToKg(a),
            )[0] ?? null
        );
