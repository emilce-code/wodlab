...
function createVariant(
  movements: ReturnType<
    typeof createWorkoutMovement
  >[] = [],
) {
  return {
    id: VARIANT_ID,
...
function setupCreateResult(
  ...
  prisma.workout.findUnique
    .mockResolvedValue(
      createWorkout(resultTypeKey),
    );

  prisma.workoutVariant.findFirst
  ...
  prisma.workoutResult.findUniqueOrThrow
    .mockResolvedValue(returnedResult);
}
...
describe('WorkoutsService', () => {
  ...
  describe('test harness', () => {
    ...
    it('executes Prisma calls through the mock', async () => {
      ...
      const result =
        await service.findWorkoutTypes();
  ...
  describe('createResult', () => {
    ...
    it('rejects unsupported result types', async () => {
      ...
      await expect(
        ...
        'Unsupported result type "UNKNOWN"',
      );
    });
  });
  ...
  describe('updateResult', () => {
    function setupUpdateResult(
      ...
      prisma.workoutResult.findFirst
        .mockResolvedValue(
          existingResult,
        );
    ...
    it('replaces performed and generated movement results when editing', async () => {
      ...
      expect(
        prisma.movementResult.deleteMany,
      ).toHaveBeenCalledWith({
        where: {
          sourceWorkoutResultId:
            RESULT_ID,
  ...
  describe('findResultSummary personal best', () => {
    ...
    it('selects the lowest TIME result as PB', async () => {
      ...
      const result =
        await service.findResultSummary(
          USER_ID,
          WORKOUT_ID,
        );

      expect(
        result.personalBest.id,
      ).toBe('result-310');
    ...
    it('selects rounds before reps for ROUNDS_REPS PB', async () => {
      ...
      expect(
        result.personalBest.rounds,
      ).toBe(8);

      expect(
        result.personalBest.reps,
      ).toBe(2);
    });
